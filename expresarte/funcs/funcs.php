<?php
	...
	function isNull($nombre,$apellido, $email, $pass, $pass_con, $telefono, $cp,$calle){
		if(strlen(trim($nombre)) < 1 || strlen(trim($apellido)) < 1 || strlen(trim($email)) < 1 || strlen(trim($pass)) < 1 || strlen(trim($pass_con)) < 1  || strlen(trim($telefono)) < 1 || strlen(trim($cp)) < 1  || strlen(trim($calle)) < 1)
		{
			return true;
			} else {
			return false;
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->isSMTP();
		$mail->SMTPAuth = true;
		$mail->SMTPSecure = 'tls';//tipo de seguridad tls (para gmail)
		$mail->Host = 'smtp.gmail.com';
		$mail->Port = '587';
		
		$mail->Username = 'deep.wounds.jz@gmail.com';
		$mail->Password = 'worldwideweb';
		
		$mail->setFrom('deep.wounds.jz@gmail.com', 'The Rock Lab');
